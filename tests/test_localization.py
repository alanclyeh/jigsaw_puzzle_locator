...
KNOWN_HARD = {
    ...
    # 速度取捨：金字塔 coarse-to-fine（<5s）在 Level-0 降解析度（48px 單尺度）篩選候選，
    # 犧牲了「近平手片」所需的全解析度多尺度鑑別力。grabCut 去背已固定 RNG seed 決定性化
    # （detector._grabcut_refine），故下列為「決定性」掉片、非 run 間翻動。Phase 1 全解析度
    # 版本可命中，屬已知速度↔近平手準度取捨。strict=False：日後若調 coarse 改善則 XPASS。
    "pieces_c2_r32":  "近平手 row32 c2↔c24 水平歧義：48px 單尺度 coarse 分不出（決定性掉）",
}


def _collect_real_piece_cases():
    cases = []
    for p in sorted((DATA_DIR / "eval_native").glob("pieces_c*_r*.jpg")):
