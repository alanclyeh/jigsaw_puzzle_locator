...
# 縮小大圖至最大邊 1600 以便快速特徵提取且具備高 Lowe's ratio 成功率
...
print(f"大圖特徵點數 (已降採樣): {len(kp_ref)}")
...
for filename in p_files:
    ...
    print(f"\n--- 測試 {filename} (去背後尺寸 {chosen_bgra.shape}) ---")
    
    # 測試四個旋轉角度
    for angle in [0, 90, 180, 270]:
        if angle == 0:
            rot_piece = chosen_bgra
        elif angle == 90:
            rot_piece = cv2.rotate(chosen_bgra, cv2.ROTATE_90_CLOCKWISE)
        elif angle == 180:
            rot_piece = cv2.rotate(chosen_bgra, cv2.ROTATE_180)
        elif angle == 270:
            rot_piece = cv2.rotate(chosen_bgra, cv2.ROTATE_90_COUNTERCLOCKWISE)
        ...
        if des_p is not None and des_ref is not None:
            ...
            print(f"  旋轉 {angle}°: kp_piece={len(kp_p)}, good={len(good)}, inliers={inliers}")
            if inliers >= 8:
                # 計算在大圖上的位置
                ...
                c = int(avg_x / gw) + 1
                r = int(avg_y / gh) + 1
                print(f"    [匹配成功] 大圖原坐標: x={avg_x:.1f}, y={avg_y:.1f} -> 網格: Row {r}, Col {c}")
