#!/usr/bin/env python3
"""啟動拼圖定位助手 Web App（正式版，唯一的 server）。
...
用法：
    ...
    python scripts/run_webapp_server.py --no-tls   # http（桌機 localhost；請用 http:// 連，勿用 https://）
...
def main():
    ...
    ip = local_ip()
    if args.no_tls:
        print(f"\n→ 桌機開啟：http://localhost:{args.port}（請用 http://，勿用 https://）\n")
