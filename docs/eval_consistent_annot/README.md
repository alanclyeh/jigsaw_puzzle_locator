...
> 註：標註 PNG 為產出物，**不入版控**（已 gitignore），僅保留本機。
> 重新產生：對 `data/eval_consistent/` 各片跑 `scripts/locate_piece.py` 取 `annotated_reference`，或用既有產生腳本輸出至本目錄。下表已記錄各片結果，無需圖片即可查閱。
...
| annot_pieces_c6_r15.png | r15c6 / r15c6 | 精確 | 洋紅(保守) |
| annot_pieces_c17_r12.png | r12c18 / r12c17 | ±1 | 綠 |
| annot_pieces_c3_r16.png | r17c2 / r16c3 | ±1 | 綠 |
...
- **合格(±10 或誠實退讓)：10/10 = 100%**；嚴格 **精確 5/10、±1 命中 7/10**。
- 仍有 2 片呈「**行(col)抓對、列(row)飄 ~10 格**」（c19_r21, c9_r27）：此參考圖（FRIEREN 海報）**縱向相似度高**，造成縱向 aliasing——單片外觀在縱向不夠獨特，屬已知單片限制（需多片全域約束才可破）。
- **標籤更正（2026-06）**：c3_r16、c6_r15 兩片原檔名誤標為 r26/r25，曾被計為 ±9/±10「失敗」並列入上述 aliasing 例子；更正列號後，系統預測 r17c2、r15c6 其實分別為 **±1 與精確命中**，反證 r16/r15 才是真值。aliasing 真實失敗案例因此由 4 片降為 2 片。
