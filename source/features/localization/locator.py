...
def _global_pose_sweep(
    ...
    全圖姿態掃描（空間影像金字塔 coarse-to-fine 兩層）：
      Level-0 粗定位（grid_px/2，~¼ 像素）：以 coarse_step(12°) 全 360 度 × 多尺度做
                   全圖帶遮罩匹配，逐網格取最高分產生候選。此層只「篩選候選格＋粗略角度」，
                   不決定最終排名，故可在低解析度跑（~4× 便宜）。
      Level-1 精修（grid_px，預設 64）：僅對 Level-0 分數 top_n_refine(K=80) 格，在各候選
                   ...
                   Level-0 降採樣影響。±12° 帶寬涵蓋 Level-0 在 32px 下的角度量化誤差。
    回傳的 acc 只含「被精修格」的全解析度分數（未精修格留 -2，上層因 score≤-1 自動跳過），
    確保排名由全解析度分數決定（Level-0 32px 的 ZNCC 系統性偏高，若混入排名會壓過真格）。
    「分數飽和」不可解偵測改用 Level-0 全網格粗分分佈，獨立計算後一併回傳。
    回傳 (分數累積圖, 姿態索引圖, 姿態表, 降採樣比例 RS=Level-1, 粗分飽和格數 coarse_plateau)；
    姿態表項目: (angle, ds, rw, rh)。

    設計脈絡：階段A 全圖粗掃原為主成本（實測 17.7s@64）；改成 Level-0 在 grid_px/2 粗掃
    可再省 ~4×。實測角度步進放寬至 18°/尺度砍至 1 檔會讓近平手案例排名翻轉、eval_native
    掉片，故保留 12°×3 尺度；改降「空間/候選篩選」解析度（與角度/尺度取樣正交）以提速。
