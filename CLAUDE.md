...
- **啟動拼圖定位助手 Web App（locator server，唯一的 server）**：`python scripts/run_webapp_server.py`（手機 https）或 `--no-tls`（桌機 localhost，請用 http:// 連）。完整流程：建立專案 → 拍單片 → 建議位置 → 確認標記 → 全圖/進度。後端 FastAPI + SQLite（`data/webapp/jp.db`），規格見 [doc/webapp_spec.md](file:///Users/alan.yeh/Workspace/my_projects/jp_locator/doc/webapp_spec.md)。
